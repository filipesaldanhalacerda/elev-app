...
// src/index.ts
var app = new Hono2();
var svcOf = /* @__PURE__ */ __name((env) => createClient(env.SUPABASE_URL, env.SERVICE_ROLE_KEY), "svcOf");
...
__name(randomPassword, "randomPassword");
var maskEmail = /* @__PURE__ */ __name((email) => `${email.split("@")[0]}@\u2026`, "maskEmail");
async function audit(svc, category, event, detail, actor) {
  await svc.from("audit_log").insert({ category, event, detail, actor: actor?.id ?? null, actor_name: actor?.name ?? null });
}
__name(audit, "audit");
async function findValidCode(svc, code) {
  const hash = await sha2563(code.toUpperCase());
  const { data } = await svc.from("access_codes").select("id, user_id, expires_at, used_at, profiles!access_codes_user_id_fkey(id, name, email, advisor_code, role, is_active)").eq("code_hash", hash).is("used_at", null).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).maybeSingle();
  return data;
}
...
app.post("/api/auth/code/validate", async (c) => {
  ...
  if (!code || code.length !== 6) return c.json({ error: "C\xF3digo deve ter 6 caracteres." }, 400);
  const svc = svcOf(c.env);
  const found = await findValidCode(svc, code);
  const profile = found?.profiles;
  if (!found || !profile || !profile.is_active) {
    return c.json({ error: "C\xF3digo inv\xE1lido ou expirado. Pe\xE7a um novo ao administrador." }, 404);
  }
...
app.post("/api/auth/code/set-password", async (c) => {
  ...
  if (!password || password.length < 8 || !/[A-Z]/.test(password) || !/\d/.test(password)) {
    return c.json({ error: "A senha n\xE3o cumpre os tr\xEAs requisitos." }, 400);
  }
  const svc = svcOf(c.env);
  const found = await findValidCode(svc, code ?? "");
  const profile = found?.profiles;
  ...
  if (error) return c.json({ error: "N\xE3o foi poss\xEDvel definir a senha." }, 500);
  await svc.from("access_codes").update({ used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", found.id);
  await audit(svc, "codigo", "C\xF3digo de acesso utilizado", `${profile.name} definiu a pr\xF3pria senha`, { id: profile.id, name: profile.name });
  return c.json({ ok: true, email: profile.email });
});
app.use("/api/admin/*", async (c, next) => {
  const token = c.req.header("Authorization")?.replace(/^Bearer /, "");
  if (!token) return c.json({ error: "N\xE3o autenticado." }, 401);
  const svc = svcOf(c.env);
  const { data, error } = await svc.auth.getUser(token);
  if (error || !data.user) return c.json({ error: "N\xE3o autenticado." }, 401);
...
app.get("/api/admin/users", async (c) => {
  const svc = c.get("svc");
  const { data: users, error } = await svc.from("profiles").select("id, name, email, advisor_code, role, is_active, created_at").order("created_at");
  if (error) return c.json({ error: error.message }, 500);
  const { data: codes } = await svc.from("access_codes").select("user_id, expires_at").is("used_at", null).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString());
...
app.post("/api/admin/users", async (c) => {
  ...
  const { name, email, advisor_code, role } = await c.req.json();
  if (!name || !email || !role) return c.json({ error: "Nome, e-mail e perfil s\xE3o obrigat\xF3rios." }, 400);
  const normalized = advisor_code ? advisor_code.toUpperCase().replace(/^A[\s-]?/, "").replace(/^0+/, "") : null;
  if (role === "advisor" && !normalized) return c.json({ error: "Assessor precisa de c\xF3digo de assessor." }, 400);
  ...
  await audit(svc, "usuario", "Usu\xE1rio criado", `${name} (${role}${normalized ? ` A-${normalized}` : ""})`, admin);
...
app.post("/api/admin/users/:id/code", async (c) => {
  ...
  if (!target) return c.json({ error: "Usu\xE1rio n\xE3o encontrado." }, 404);
  await svc.from("access_codes").update({ used_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("user_id", userId).is("used_at", null);
  ...
  const { error } = await svc.from("access_codes").insert({
    user_id: userId,
    code_hash: await sha2563(code),
    expires_at: expiresAt2,
    created_by: admin.id
  });
  if (error) return c.json({ error: error.message }, 500);
  await audit(svc, "codigo", "C\xF3digo de acesso gerado", `para ${target.name} \u2014 senha anterior invalidada`, admin);
  return c.json({ code, expires_at: expiresAt2 });
});
app.patch("/api/admin/users/:id", async (c) => {
  ...
  if (body.is_active === false) {
    ...
    await audit(svc, "usuario", "Usu\xE1rio desativado", target.name, admin);
  } else if (body.is_active === true) {
    await svc.auth.admin.updateUserById(userId, { ban_duration: "none" });
    await audit(svc, "usuario", "Usu\xE1rio reativado", target.name, admin);
  } else {
    await audit(svc, "usuario", "Usu\xE1rio editado", target.name, admin);
  }
...
// .wrangler/tmp/bundle-DUBpEe/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DUBpEe/middleware-loader.entry.ts
